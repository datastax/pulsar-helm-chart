...
issuerUri="https://dev-42506116.okta.com/oauth2/aus7ypk6sjvgF4l615d7"
authParams=$(cat <<EOF
{"privateKey":"/pulsar/conf/creds.json","issuerUrl":"$issuerUri","scope":"pulsar_client_m2m","audience":"api://pulsarClient"}
EOF
)

### Use Pulsar client with non-TLS endpoint in Pulsar with OIDC:
bin/pulsar-perf produce -r 1000 --size 1024 --auth-plugin "org.apache.pulsar.client.impl.auth.oauth2.AuthenticationOAuth2" --auth-params $authParams --service-url pulsar://pulsar-proxy.pulsar.svc.cluster.local:6650/ persistent://public/default/test
### Use Pulsar client with TLS endpoint in Pulsar with OIDC:
bin/pulsar-perf produce -r 1000 --size 1024 --auth-plugin "org.apache.pulsar.client.impl.auth.oauth2.AuthenticationOAuth2" --auth-params $authParams --service-url pulsar+ssl://pulsar-proxy.pulsar.svc.cluster.local:6651/ persistent://public/default/test

bin/pulsar-admin --auth-plugin "org.apache.pulsar.client.impl.auth.oauth2.AuthenticationOAuth2" --auth-params $authParams namespaces policies public/default
...
cat << EOF > /pulsar/kafka/kafka_2.12-3.3.2/config/producer.properties
